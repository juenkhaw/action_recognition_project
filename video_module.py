...
def load_video(frames_path, modality, scale_h, scale_w, output_h, output_w, output_len):
    """
    ...
    while count < t_index[1]:
        if frame_chn == 3:
            ...
            buffer_frame = cv2.imread(frames_path + '/' + path_content[count], cv2.IMREAD_GRAYSCALE)
                
        # resize the frame
        # add channel dimension if frame is flow
        buffer_frame = cv2.resize(buffer_frame, (scale_w, scale_h))
        if frame_chn == 1:
            buffer_frame = buffer_frame[:, :, np.newaxis]
...
if __name__ == '__main__':
    ...
    buffer = load_video(video_path, 'flow', 128, 171, 112, 112, 16)
