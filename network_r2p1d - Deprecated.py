...
class R2Plus1DNet(nn.Module):
    ...
    def __init__(self, layer_sizes, num_classes, device, block_type = SpatioTemporalResBlock, 
                 in_channels = 3, final_endpoint = 'Logits', 
                 bn_momentum = 0.1, bn_epson = 1e-3, 
                 name = 'R2+1D', verbose = 'True'):
        ...
        self.end_points[self.VALID_ENDPOINTS[5]] = nn.Linear(512, num_classes)
        
        # appending parameters from all layers of operations
        self.my_param = nn.ParameterList()
        for end in self.VALID_ENDPOINTS:
            if end in self.end_points.keys():
                module = self.end_points[end]
                while not isinstance(module, nn.Module):
                    for m in module.modules():
                        self.my_param.append(m.weight)
                        if m.bias is not None:
                            self.my_param.append(m.bias)
